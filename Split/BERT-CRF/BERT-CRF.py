...
from conlleval import reporter
...
if os.name == 'nt':
    ...
    bert_path = '/CHIS/BERT-CRF/BERT_ENGLISH_DIR'
    root_path = ''
...
flags.DEFINE_string(
    "task_name", 'CHIS', "The name of the task to train."
)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, "label_test0.utf8")
        ...
        with codecs.open(os.path.join(FLAGS.output_dir, 'predicted_result.txt'), 'a', encoding='utf-8') as fd:
            fd.write(''.join(eval_result))
        '''

        file = open("./output/result_cut/label_test0.utf8", "r", encoding="gbk")
        file2 = open("./output/result_cut/label_test.utf8", "w", encoding="utf-8")
        ...
        crf_convert('./output/result_cut/label_test.utf8', './output/result_cut/output.txt')
        ...
        def crf_convert(input_file, output_file):
            ...
            for line in inf.readlines():
                ...
                if word_list[-1] != '@':
                    if word_list[-1] == 'S' or word_list[-1] == 'E':
                        out.write(word_list[0] + ' ')
                    if word_list[-1] == 'B' or word_list[-1] == 'M':
                        out.write(word_list[0])
                if word_list[-1] == '@':
                    out.write('\n')
            out.close()
        ...
        def deal_file(filename_old, filename_new):
            try:
                with open(filename_old, encoding="utf8") as f1:
                    with open(filename_new, "w", encoding="utf8") as f2:
                        for i in f1:
                            if i.strip(): f2.write(replace_comma(i))
            except BaseException as e:
                print(e)
        deal_file('./output/result_cut/output.txt', './output/result_cut/predict_out.txt')
