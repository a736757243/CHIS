...
def report_notprint(counts, out=None):
    ...
    num = 0
    count = 0
    with open("./output/result_cut/predict_out.txt") as file01:
        for line in file01:
            num = num + 1
    with open("./output/result_cut/predict_out.txt", 'r') as x, open("./PEOPLEdata2/tst.txt", 'r') as y:
        line1 = x.readlines()
        line2 = y.readlines()
        for i in range(num):
            if line1[i] == line2[i]:
    ...
    with open("./output/result_cut/predict_out.txt", 'r') as x, open("./PEOPLEdata2/tst.txt", 'r') as y:
        ...
        for i in range(num):
            if line1[i] == line2[i]:
                ...
                var = str(i + 1)
                lines.append("原单词:" + line2[i])
                lines.append("拆分词:" + line1[i] + '\n')
    s = ''.join(lines)
    f = open("./output/result_cut/different.txt", 'w+')
    f.write(s)
    del lines[:]
    print("正确拆分的标识符个数：" + str(count))
    print("未正确拆分的标识符个数：" + str(num - count))
    # accuracy = (count/num)
    # print ('%.2f' %(a/b))
    # print("accuracy:"+str(accuracy))
    print("accuracy:" + '%.f' % (100 * count / num) + "%")
